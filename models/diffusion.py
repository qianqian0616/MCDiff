...
def compute_alpha(beta, t):
    ...
    a = (1 - beta).cumprod(dim=0).index_select(0, t + 1).view(-1, 1, 1)   # 连乘
    return a

def generalized_steps(x, con_x, seq, model, b, **kwargs):
    with torch.no_grad():
        n = x.size(0)
        ...
        for i, j in zip(reversed(seq), reversed(seq_next)):
            ...
            xt = xs[-1].to(device)
            et = model(torch.cat([con_x, xt], dim=1), t)
...
class Diffusion(object):
    def __init__(self, args):
        ...
        alphas_cumprod_prev = torch.cat([torch.ones(1).to(self.device), alphas_cumprod[:-1].to(self.device)], dim=0)
    ...
    def train(self, x, con_x):  # bs*3*144*72
        n = x.size(0) 
        x = x.to(self.device)
        #x = 2 * x - 1.0
        ...
        b = self.betas
        # antithetic sampling
        #t = torch.randint(low=0, high=self.num_timesteps, size=(1, )).to(self.device)
        t = torch.randint(low=0, high=self.num_timesteps, size=(n // 2 + 1, )).to(self.device)
        t = torch.cat([t, self.num_timesteps-t-1], dim=0)[:n]
        return self.noise_estimation_loss(x, con_x, t, e, b)

    def noise_estimation_loss(self, x0, con_x, t, e, b, keepdim=False):
        # 根据betas和时间步骤t计算扩散系数alpha
        a = (1-b).cumprod(dim=0).index_select(0, t).view(-1, 1, 1)
        # 使用扩散模型生成扰动样本x
        x = x0 * a.sqrt() + e * (1.0 - a).sqrt()
     
        output = self.model(torch.cat([con_x, x], dim=1), t.float())
        # 计算噪声估计损失
        if keepdim:
    ...
    def sample(self, x0, con_x, last=True):
        # 生成随机噪声
        ...
        # 用于采样的广义步骤
        xs = generalized_steps(e, con_x, seq, self.model, self.betas, eta=self.args.eta)
        x = xs
        if last:
            x = x[0][-1]
        return x
